...
public class HelloVideoRenderer implements GLSurfaceView.Renderer {
    ...
    public void onDrawFrame(GL10 gl10) {
        ...
        // dylan added code below:
        Bitmap b = Bitmap.createScaledBitmap(Util.createBitmapFromGLSurface(0,0,1280,720,gl10),426,240,false);

            try {
                if (data != null && ois != null){
                    System.out.println(b.getConfig().toString());
                    ois.writeObject(new tangoPacket(Util.bmpToBarr(b),HelloVideoActivity.depth));
                }else{
                    data = new Socket("192.168.42.184",1026);
                    ois = new ObjectOutputStream(data.getOutputStream());
                }
